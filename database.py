...
class UserModel:
    def __init__(self, db_conn=None):
        # We don't strictly need the connection here since
        # get_db_connection() is available globally in the module
        pass

    def get_or_create_supabase_user(self, supabase_user, access_control):
        """Syncs a Supabase Auth user with the local SQLite users table"""
        conn = get_db_connection()
        ...
        is_admin = email.lower() in [
            e.lower() for e in access_control.get("admin_emails", [])
        ]

        try:
            # 1. Check if user exists
            user = conn.execute(
                "SELECT * FROM users WHERE oidc_sub = ?", (supabase_id,)
            ).fetchone()
            ...
            if user:
                ...
                conn.execute(
                    """
                    UPDATE users SET
                        last_login = ?, is_admin = ?, full_name = ?
                    WHERE oidc_sub = ?
                """,
                    (datetime.now().isoformat(), is_admin, full_name, supabase_id),
                )
            else:
                # 3. Create new
                conn.execute(
            ...
            # Fetch the final record to return (including the local integer ID)
            updated_user = conn.execute(
                "SELECT * FROM users WHERE oidc_sub = ?", (supabase_id,)
            ).fetchone()

            # Convert to dict and ensure key names match app.py expectations
            result = dict(updated_user)
            result["supabase_id"] = result["oidc_sub"]
            return result
